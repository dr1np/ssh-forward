...
构建结果位于 `artifacts/v0.2.1/`。Windows portable 压缩包内只有 `SSHForwarder.exe` 和使用说明，不再有 `ssh-forwarder-service.exe`。Linux 的 AppImage/deb 可以使用 `scripts/Dockerfile.linux` 中的工具链构建；macOS 需要在 macOS 主机上运行相同的 Tauri 构建命令。
