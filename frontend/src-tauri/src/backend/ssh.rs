...
mod tests {
    ...
    fn config_profile_uses_alias_and_forward_options() {
        ...
        assert!(command.iter().any(|item| item == "ExitOnForwardFailure=yes"));
        assert!(command.windows(2).any(|items| items == ["-L", "127.0.0.1:15432:127.0.0.1:5432"]));
    ...
    fn custom_profile_includes_user_port_key_and_config_file() {
        ...
        assert!(command.windows(2).any(|items| items == ["-i", "C:/keys/work key"]));
        assert!(command.windows(2).any(|items| items == ["-F", "C:/tmp/ssh_config"]));
    ...
    fn ipv6_forward_spec_is_bracketed() {
        ...
        let command = build_ssh_command("ssh", &profile, None).unwrap();
        assert!(command.windows(2).any(|items| items == ["-L", "[::1]:8080:[2001:db8::1]:443"]));
    }
}
use super::endpoint::format_forward_spec;
use super::model::{ConnectionType, ForwardProfile};

pub fn build_ssh_command(ssh_executable: &str, profile: &ForwardProfile, config_file: Option<&str>) -> Result<Vec<String>, String> {
    profile.validate()?;
    let mut command = vec![
        ...
        "-L".into(), format_forward_spec(profile.local_bind.as_str(), profile.local_port, &profile.remote_host, profile.remote_port),
    ];
